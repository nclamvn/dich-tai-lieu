...
class GhostwriterAgent(BaseAgent):
    """
    Ghostwriter Agent (formerly AuthorEngine) providing:
    ...
    def __init__(
        ...
        """
        super().__init__(agent_id, "ghostwriter", llm_provider)
    ...
    async def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        {
            "action": "propose" | "rewrite" | "expand" | "chapter" | "brainstorm" | "critique",
            "params": { ... }
        }
        """
        ...
        if action == "propose":
            result = await self.propose_next_paragraph(**params)
            # Serialize if result is list of objects
            if isinstance(result, list):
                return {"variations": [v.to_dict() if hasattr(v, "to_dict") else v for v in result]}
            return {"result": result}

        elif action == "rewrite":
            result = await self.rewrite_paragraph(**params)
            return {"result": result}

        elif action == "expand":
            result = await self.expand_idea(**params)
            return {"result": result}
            
        elif action == "chapter":
             result = await self.generate_chapter(**params)
             return {"result": result}

        elif action == "brainstorm":
            result = await self.brainstorm_ideas(**params)
            return {"ideas": result}

        elif action == "critique":
            result = await self.critique_text(**params)
    ...
    async def propose_next_paragraph(
        ...
        """
        # Phase 4.3: Memory Retrieval
        memory_context = ""
        if project_id and author_id and current_chapter is not None:
            try:
                project = self.load_project(author_id, project_id)
                if project:
                    # 1. Structured Memory (Characters, Plot)
                    if project.memory:
                        memory_context += project.memory.get_context_for_chapter(current_chapter)
                    
                    # 2. Vector Memory (Recent Narrative Flow)
                    if project.vector_memory:
                        recent = project.vector_memory.get_recent_context(current_chapter)
                        if recent:
                            memory_context += f"\n\n[Previous Narrative Context]\n{recent}"
                            
                    if memory_context:
                        # Prepend memory context to the prompt context
                        context = f"STORY BIBLE & MEMORY:\n{memory_context}\n\nCURRENT MANUSCRIPT:\n{context}"
            except Exception as e:
                # Log but don't fail if memory fails
    ...
    async def propose_next_paragraph_stream(
        self,
        context: str,
        instruction: str = "Continue writing naturally",
        style: str = None,
        ...
        """
        Generate next paragraph as a stream (Single variation only)
        """
        # Memory Retrieval (reused logic)
        memory_context = ""
        if project_id and author_id and current_chapter is not None:
            try:
                project = self.load_project(author_id, project_id)
        ...
        # Force single variation for streaming
        prompt = CO_WRITE_PROMPT.format(
            context=context,
            style_instruction=style_instruction,
            instruction=instruction,
    ...
    async def _call_llm_stream(self, prompt: str):
        ...
        if self.llm_client is None:
            # Simulated stream if no client
            full_text = "[LLM Response Placeholder]"
        ...
        if hasattr(self.llm_client, "generate"):
            ...
            raise ValueError("LLM provider does not support streaming")
